...
- All keys are stored in **GitHub Actions Secrets**, never committed.  
- Workflow never prints keys to logs.  
...
It fetches your SUPABASE_PROJECTS_JSON secret, parses it, and for each project:

Calls
https://your-project.supabase.co/auth/v1/health

Sends the anon key as apikey

Logs if the project responded correctly

Supabase registers this as activity, preventing auto-pausing.
...
Yes. Just add all projects to the JSON list.

Can I make the repo public?
Yes — all sensitive data stays in GitHub Secrets.

Does this violate any Supabase rules?
No. It only uses public anon endpoints, same as any frontend app.

❤️ Contribute
This project is intentionally tiny.
Feel free to open issues or PRs to add optional features like:
